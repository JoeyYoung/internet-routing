...
class T:
    ...
    def learn_exp_glb(self, exps, indexs_learn):
        for k in indexs_learn:
            exp = exps[k]
            td_error = self.critic.learn(exp[0], exp[3], exp[1])
            self.actor.learn(exp[4], exp[2], td_error)

    def learn_exp_loc(self, exps, indexs_learn):
        for k in indexs_learn:
            exp = exps[k]
            td_error = self.critic.learn(exp[4], exp[3], exp[1])
            self.actor.learn(exp[4], exp[2], td_error)
