...
"""
    Used for a defined topology based on LHCORN reality
"""
...
class HugeGenerator:
    ...
    # ID 1-13
    def add_m_nodes(self):
        # add M provider, m_peers
        for i in range(1, 14):
            self.connect_m_t(i, 0)
        for i in range(1, 13):
            self.connect_mm_peer(i, i + 1)
        self.connect_mm_peer(13, 1)

    def add_cp_nodes(self):
        # add cp-m provider, cp-t provider, cp-cp peer, fixed relationship
        for region in range(1, 14):
            header_cp = 14 + 6 * (region - 1)
            tail_cp = 19 + 6 * (region - 1)
            for i in range(header_cp, tail_cp + 1):
                self.connect_cp_m(i, region)
                for k in range(1, 4):
                    m = (region + k) % 13
                    if m == 0:
                        m = 13
                    self.connect_cp_m(i, m)
            self.connect_cp_t(header_cp, 0)
            self.connect_cp_t(tail_cp, 0)

            peer_cp_1 = header_cp + 1
            peer_cp_2 = header_cp + 2
            self.connect_cpcp_peer(peer_cp_1, peer_cp_2)

            out = (region + 1) % 13
            if out == 0:
                out = 13
            header_cp_out = 14 + 6 * (out - 1)
            peer_cp_3 = header_cp_out + 1
            self.connect_cpcp_peer(peer_cp_1, peer_cp_3)
    ...
    """
        Used to build link based on node id
    """
    ...
    def connect_cpcp_peer(self, cpid1, cpid2):
        self.ids[cpid1].peers_CP.append(self.ids[cpid2])
        self.ids[cpid2].peers_CP.append(self.ids[cpid1])
