...
class Game:
    ...
    def play_game(self):
        ...
        for i in self.RLs:
            ...
            n_features_critic = len(states[i.id]) + 1
                    ...
                    for agent in self.Ns:
                        ...
                        if len(self.experience_pool[agent.id]) < self.pool_size:
                            ...
                            for k in indexs_learn:
                                ...
                                td_error = agent.critic.learn(exp[4], exp[3], exp[1])
                                agent.actor.learn(exp[4], exp[2], td_error)
                    flow_num += 1

            sums.append(sum_all)
            print('game30-mask-local: ' + str(sum_all))
            if time % 3000 == 0 and time != 0:
                str1 = 'game30-mask-local' + str(time) + '.txt'
                file = open(str1, 'w')
                file.write(str(sums))
                file.close()
