...
class Game:
    ...
    def play_game(self):
        ...
        for time in range(self.MAX):
            # if time % 3000 == 0 and time != 0 and self.episode > 0.4:
            #     self.episode -= 0.1
            ...
            flow_num = 0
            # can store round into experience pool, and train then but not based on each flow like this
            for i in TF.keys():
                for j in TF[i].keys():
                    for agent in self.Ns:
            ...
            sums.append(sum_all)
            print('game30-mask-glb: ' + str(sum_all))
            if time % 3000 == 0 and time != 0:
                str1 = 'game30-mask-glb' + str(time) + '.txt'
                file = open(str1, 'w')
                file.write(str(sums))
                file.close()
