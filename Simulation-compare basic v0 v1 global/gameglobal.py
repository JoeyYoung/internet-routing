...
class Game:
    ...
    def play_game(self):
        ...
        for i in range(self.N):
            for j in TF.keys():
                for k in TF[j].keys():
                    if j in range(len(self.generator.Ts)) and k in range(len(self.generator.Ts)):
        ...
        for time in range(self.MAX):
            ...
            for i in range(self.N):
                for j in TF.keys():
                    for k in TF[j].keys():
                        states_[i].append(ee_throughput[j][k])
                for q in range(self.N):
                    states_[i].append(actions[q])
